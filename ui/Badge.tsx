...
export type BadgeVariant = 'default' | 'primary' | 'outline' | 'fruit' | 'bloem' | 'blad' | 'wortel';

const BADGE_VARIANTS: Record<BadgeVariant, { bg: string; color: string; border: string }> = {
  default:  { bg: 'var(--color-surface)', color: 'var(--color-gray)',  border: 'var(--color-border)' },
  primary:  { bg: 'var(--color-primary)', color: '#fff',               border: 'var(--color-primary)' },
  outline:  { bg: 'transparent',          color: 'var(--color-on-surface)', border: 'var(--color-border)' },
