...
import { POPULAIRE_DRUIVEN, getSuggesties, type DruifInfo } from '../data/druiven-database';
...
export function DruivenInput({ druiven, onChange, wijnType, land, regio }: DruivenInputProps) {
  ...
  const verwijderDruif = (index: number) => onChange(druiven.filter((_, i) => i !== index));

  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      if (showSuggesties && suggesties.length > 0) voegDruifToe(suggesties[geselecteerdeIndex].naam);
  ...
  useEffect(() => { setGeselecteerdeIndex(0); }, [inputValue]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
          {druiven.map((druif, index) => (
            <span key={index} style={{
              ...
              padding: '0.25rem 0.5rem', fontFamily: 'var(--font-body)', fontSize: '0.72rem', fontWeight: 600,
              background: 'var(--color-surface-high)', border: '2px solid var(--color-border)', color: 'var(--color-on-surface)',
            }}>
              {druif}
              <button type="button" onClick={() => verwijderDruif(index)}
      ...
      <div style={{ position: 'relative' }}>
        <input
          ...
          style={{
            width: '100%', padding: '0.65rem 0.875rem', fontFamily: 'var(--font-body)', fontSize: '0.95rem',
          ...
          <div role="listbox" style={{
            ...
            {suggesties.slice(0, 10).map((druif, index) => (
              <button
                ...
                onMouseDown={(e) => { e.preventDefault(); voegDruifToe(druif.naam); }}
                style={{
                  width: '100%', textAlign: 'left', padding: '0.6rem 0.875rem',
                  background: index === geselecteerdeIndex ? 'var(--color-surface)' : 'transparent',
