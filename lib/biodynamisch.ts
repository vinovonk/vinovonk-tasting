...
> = {
  fruit: {
    label: 'Fruit dag',
    beschrijving: 'Wijn is open, fruitig en expressief. De beste omstandigheden om te proeven en te beoordelen.',
  ...
  bloem: {
    label: 'Bloem dag',
    beschrijving: 'Wijn is aromatisch en fris. Een goede dag om te proeven, vooral voor witte en aromatische wijnen.',
  ...
  wortel: {
    label: 'Wortel dag',
    beschrijving: 'Wijn smaakt plat, tannineus of gesloten. Vermijd serieuze beoordelingen op wortel dagen (aanbeveling Cees van Casteren MW).',
...
export type MaanFase = 'nieuw' | 'wassend_sikkel' | 'wassend_kwart' | 'wassend_bol' | 'vol' | 'afnemend_bol' | 'afnemend_kwart' | 'afnemend_sikkel';
...
const MAANFASE_DATA: Record<MaanFase, Omit<MaanFaseInfo, 'fase' | 'illuminatie' | 'leeftijd'>> = {
  nieuw: {
    label: 'Nieuwe maan',
    emoji: '🌑',
    beschrijving: 'Maan staat tussen aarde en zon in — niet zichtbaar. Begin van nieuwe cyclus.',
    invloedOpWijn: 'Wijn kan gesloten zijn. Minder geschikt voor serieuze beoordeling.',
  },
  wassend_sikkel: {
    label: 'Wassende sikkel',
    emoji: '🌒',
    beschrijving: 'Kleine lichtende sikkel rechts. Energie begint te stijgen.',
    invloedOpWijn: 'Wijn wordt expressiever. Beginnend goed moment voor proeven.',
  },
  wassend_kwart: {
  ...
  vol: {
    ...
    invloedOpWijn: 'Maximale energie. Wijn op volle expressie — ideaal voor beoordeling. Gecombineerd met fruit dag = perfecte omstandigheden.',
  },
  afnemend_bol: {
    label: 'Afnemende bol',
    emoji: '🌖',
  ...
  afnemend_kwart: {
    ...
    invloedOpWijn: 'Wijn begint te sluiten. Minder ideaal voor serieuze beoordeling.',
  },
  afnemend_sikkel: {
    label: 'Afnemende sikkel',
    emoji: '🌘',
    beschrijving: 'Kleine sikkel links. Einde van cyclus nadert.',
    invloedOpWijn: 'Wijn gesloten of ingetogen. Vermijd belangrijke beoordelingen.',
  },
};
...
export function getMaanFaseInfo(datum: Date): MaanFaseInfo {
  ...
  // Illuminatie (0–100%)
  const illuminatie = Math.round((1 - Math.cos((2 * Math.PI * leeftijd) / synodicPeriod)) / 2 * 100);
