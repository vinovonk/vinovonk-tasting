...
import type { TastingSession, SessionSummary, TastingNote } from '../types';
...
export function getSessions(): SessionSummary[] {
  return getIndex().sort((a, b) => new Date(b.datum).getTime() - new Date(a.datum).getTime());
}
...
export function createSession(naam: string, datum?: string, beschrijving?: string): TastingSession {
  const id = uuidv4();
  ...
  index.push({ id, naam, datum: sessionDatum, aantalFlessen: 0, createdAt: now });
  setIndex(index);
...
export function exportAllData(): string {
  ...
  return JSON.stringify({ version: 1, exportDate: new Date().toISOString(), sessions }, null, 2);
}

export function importData(json: string): { imported: number; errors: string[] } {
  const errors: string[] = [];
  let imported = 0;

  try {
    const data = JSON.parse(json);
    const sessions: TastingSession[] = data.sessions ?? (Array.isArray(data) ? data : []);

    for (const session of sessions) {
      if (!session.id || !session.naam) {
        errors.push(`Sessie overgeslagen: geen id of naam`);
        continue;
      }
