...
export const FL = {
  nl: {
    ...
    kwaliteitBlic: 'Kwaliteitsniveau — BLIC (Balans, Lengte, Intensiteit, Complexiteit)',
    ...
    gerechtCombinatie_placeholder: 'Bijv. Ossobuco, gegrilde entrecote, of puur',
    ...
    naamCuvee_placeholder: 'bijv. Blanc de Blancs, Brut Réserve, La Grande Dame...',
    producent_champagne: 'Producent / Maison',
    producent_champagne_placeholder: 'bijv. Moët & Chandon, Krug, Billecart-Salmon...',
    ...
    vergelijking_placeholder: 'Hoe verhoudt dit zich tot de alcoholische versie? Wat mist er, wat is beter?',
    ...
    // Validation
    missingFields: (n: number) => `Nog ${n} veld${n > 1 ? 'en' : ''} in te vullen`,
  ...
  en: {
    ...
    kwaliteitBlic: 'Quality level — BLIC (Balance, Length, Intensity, Complexity)',
    ...
    naamCuvee_placeholder: 'e.g. Blanc de Blancs, Brut Réserve, La Grande Dame...',
    producent_champagne: 'Producer / Maison',
    producent_champagne_placeholder: 'e.g. Moët & Chandon, Krug, Billecart-Salmon...',
    ...
    vergelijking_placeholder: 'How does this compare to the alcoholic version? What is missing, what is better?',
    ...
    // Validation
    missingFields: (n: number) => `${n} required field${n > 1 ? 's' : ''} missing`,
...
export function getHelderheidOpties(lang: Lang) {
  return lang === 'en'
    ? [{ waarde: 'helder', label: 'Clear' }, { waarde: 'troebel', label: 'Hazy' }]
    : [{ waarde: 'helder', label: 'Helder' }, { waarde: 'troebel', label: 'Troebel' }];
}

export function getIntensiteitOpties(lang: Lang) {
  return lang === 'en'
    ? [{ waarde: 'licht', label: 'Light' }, { waarde: 'medium', label: 'Medium' }, { waarde: 'uitgesproken', label: 'Pronounced' }]
    : [{ waarde: 'licht', label: 'Licht' }, { waarde: 'medium', label: 'Medium' }, { waarde: 'uitgesproken', label: 'Uitgesproken' }];
}

export function getZoetheidOpties(lang: Lang) {
  return lang === 'en'
    ? [{ waarde: 'droog', label: 'Dry' }, { waarde: 'halfdroog', label: 'Off-dry' }, { waarde: 'medium', label: 'Medium' }, { waarde: 'zoet', label: 'Sweet' }]
    : [{ waarde: 'droog', label: 'Droog' }, { waarde: 'halfdroog', label: 'Halfdroog' }, { waarde: 'medium', label: 'Medium' }, { waarde: 'zoet', label: 'Zoet' }];
}

export function getDrieSchaalOpties(lang: Lang) {
  return lang === 'en'
    ? [{ waarde: 'laag', label: 'Low' }, { waarde: 'medium', label: 'Medium' }, { waarde: 'hoog', label: 'High' }]
    : [{ waarde: 'laag', label: 'Laag' }, { waarde: 'medium', label: 'Medium' }, { waarde: 'hoog', label: 'Hoog' }];
}

export function getBodyOpties(lang: Lang) {
  return lang === 'en'
    ? [{ waarde: 'licht', label: 'Light' }, { waarde: 'medium', label: 'Medium' }, { waarde: 'vol', label: 'Full' }]
    : [{ waarde: 'licht', label: 'Licht' }, { waarde: 'medium', label: 'Medium' }, { waarde: 'vol', label: 'Vol' }];
}

export function getAfdronkOpties(lang: Lang) {
  return lang === 'en'
    ? [{ waarde: 'kort', label: 'Short' }, { waarde: 'medium', label: 'Medium' }, { waarde: 'lang', label: 'Long' }]
    : [{ waarde: 'kort', label: 'Kort' }, { waarde: 'medium', label: 'Medium' }, { waarde: 'lang', label: 'Lang' }];
}
...
export function getOpnieuwKopenOpties(lang: Lang) {
  return lang === 'en'
    ? [{ waarde: 'ja', label: 'Yes' }, { waarde: 'misschien', label: 'Maybe' }, { waarde: 'nee', label: 'No' }]
    : [{ waarde: 'ja', label: 'Ja' }, { waarde: 'misschien', label: 'Misschien' }, { waarde: 'nee', label: 'Nee' }];
}
