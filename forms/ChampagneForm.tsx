...
function ChampagneAromaPicker({
	...
	const toggle = (a: string) =>
		geselecteerd.includes(a)
			? onChange(geselecteerd.filter((x) => x !== a))
			: onChange([...geselecteerd, a]);
	const addCustom = () => {
					...
					<div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
						{neusAromas.map((a) => (
							<button
								...
								onClick={() => toggle(a)}
								style={{
									...pillStyle(geselecteerd.includes(a)),
									opacity: geselecteerd.includes(a) ? 1 : 0.4,
								}}
							>
								{a}
							</button>
					...
					<div style={{ display: "flex", flexWrap: "wrap", gap: "2px" }}>
						{cat.aromas.map((a) => (
							<button
								...
								type="button"
								onClick={() => toggle(a)}
								style={pillStyle(geselecteerd.includes(a))}
							>
								{a}
							</button>
						))}
					...
					{geselecteerd.map((a) => (
						<span
							...
							style={{
								...
								fontSize: "0.72rem",
								fontWeight: 600,
								background: neusSet.has(a)
									? "var(--color-surface-high)"
									: "var(--color-primary)",
								color: neusSet.has(a) ? "var(--color-on-surface)" : "#fff",
								border: `2px solid ${neusSet.has(a) ? "var(--color-border)" : "var(--color-primary)"}`,
							}}
						>
							{a}
							<button
...
function DruivenRassenInput({
	...
	const toggle = (r: string) =>
		druiven.includes(r)
			? onChange(druiven.filter((d) => d !== r))
			: onChange([...druiven, r]);
	const addCustom = () => {
		const t = custom.trim();
		if (t && !druiven.includes(t)) onChange([...druiven, t]);
		setCustom("");
	};
			...
			<div style={{ display: "flex", flexWrap: "wrap", gap: "2px" }}>
				{champagneDruivenRassen.map((r) => (
					<button
						...
						type="button"
						onClick={() => toggle(r)}
						style={pillStyle(druiven.includes(r))}
					>
						{r}
					</button>
				))}
			</div>
			{druiven.filter((d) => !champagneDruivenRassen.includes(d)).length >
				0 && (
				<div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
					{druiven
	...
	function ChampagneForm(
			...
			() => ({
				...
				mergeAIData: (ai) =>
					setData((prev) => ({
						...prev,
						...ai,
						visueel: { ...prev.visueel, ...(ai.visueel ?? {}) },
						neus: { ...prev.neus, ...(ai.neus ?? {}) },
						mondgevoel: { ...prev.mondgevoel, ...(ai.mondgevoel ?? {}) },
						conclusie: { ...prev.conclusie, ...(ai.conclusie ?? {}) },
					})),
			}),
			[data],
		);
		...
		const handleSave = () => {
			if (fase === "proeven") {
				...
				if (!data.visueel.kleur)
					missing.push({ label: L.kleur, tab: "visueel" });
				if (!data.visueel.belGrootte)
					missing.push({ label: L.belgrootte, tab: "visueel" });
				if (!data.visueel.mousse)
					missing.push({ label: L.mousse, tab: "visueel" });
				if (!data.neus.intensiteit)
					missing.push({ label: L.intensiteit, tab: "neus" });
				if (!data.neus.autolytischKarakter)
					missing.push({ label: L.autolytischKarakter, tab: "neus" });
				if (!data.mondgevoel.zoetheid)
					missing.push({ label: L.zoetheid, tab: "mondgevoel" });
				if (!data.mondgevoel.zuurgraad)
					missing.push({ label: L.zuurgraad, tab: "mondgevoel" });
				if (!data.mondgevoel.afdronkLengte)
					missing.push({ label: L.afdronkLengteShort, tab: "mondgevoel" });
				if (!data.conclusie.kwaliteit)
					missing.push({ label: L.kwaliteitsniveau, tab: "conclusie" });
				if (missing.length > 0) {
					const tabNames: Record<string, string> = {
						visueel: L.tabUiterlijk,
						neus: L.tabNeus,
						mondgevoel: L.mondgevoel,
						conclusie: L.tabConclusies,
					};
		...
		const prevTab = () => {
			const i = TABS_PROEVEN.indexOf(tab);
			if (i > 0) setTab(TABS_PROEVEN[i - 1]);
		};
		const nextTab = () => {
			const i = TABS_PROEVEN.indexOf(tab);
			if (i < TABS_PROEVEN.length - 1) setTab(TABS_PROEVEN[i + 1]);
		};
			...
			return (
				...
				>
					...
					>
						...
						<AutocompleteInput
							placeholder={L.naamCuvee_placeholder}
							value={data.cuveeNaam}
							onChange={(v) => setData({ ...data, cuveeNaam: v })}
							suggesties={[]}
						/>
					</div>
					<div
					...
					>
						...
						<AutocompleteInput
							placeholder={L.producent_champagne_placeholder}
							value={data.producent ?? ""}
							onChange={(v) => setData({ ...data, producent: v })}
					...
					>
						...
						<AutocompleteInput
							placeholder={L.villageRegio_placeholder}
							value={data.regio ?? ""}
					...
					<ButtonGroup
						...
						onChange={(v) =>
							setData({ ...data, cuveeType: v as typeof data.cuveeType })
						}
						size="sm"
					/>
					{(data.cuveeType === "millesime" ||
						data.cuveeType === "prestige") && (
						<Input
							label={L.jaargang_champagne}
							type="number"
							placeholder={L.jaargang_champagne_placeholder}
							value={data.jaargang ?? ""}
							onChange={(e) =>
								setData({
									...data,
									jaargang: e.target.value ? Number(e.target.value) : undefined,
								})
							}
						/>
					)}
					<ButtonGroup
						label={L.stijl}
						opties={
							stijlOpties as unknown as { waarde: string; label: string }[]
						}
						waarde={data.stijl}
						onChange={(v) =>
							setData({ ...data, stijl: v as typeof data.stijl })
						}
						size="sm"
					/>
					<ButtonGroup
						label={L.dosage}
						opties={
							dosageOpties as unknown as { waarde: string; label: string }[]
						}
						waarde={data.dosage}
						onChange={(v) =>
					...
					>
						...
						<Input
							...
							onChange={(e) =>
								setData({
									...data,
									dosageGl: e.target.value ? Number(e.target.value) : undefined,
								})
							}
						/>
					</div>
					...
					>
						...
						<DruivenRassenInput
							...
							onChange={(druiven) =>
								setData({ ...data, druivenrassen: druiven })
							}
						/>
					</div>
					<ButtonGroup
						label={L.producerType}
						opties={
							producerTypeOpties as unknown as {
								waarde: string;
								label: string;
							}[]
						}
						...
						onChange={(v) =>
							setData({ ...data, producerType: v as typeof data.producerType })
						}
						size="sm"
					/>
					<ButtonGroup
						label={L.classificatie}
						opties={
							classificatieOpties as unknown as {
								waarde: string;
								label: string;
							}[]
						}
						...
						onChange={(v) =>
							setData({
								...data,
								classificatie: v as typeof data.classificatie,
							})
						}
						size="sm"
					/>
					<Input
						label={L.dégorgementdatum}
						type="date"
						value={data.disgorgeerdatum ?? ""}
						onChange={(e) =>
							setData({ ...data, disgorgeerdatum: e.target.value })
						}
					/>
				</div>
			);
							...
							>
								<ButtonGroup
									...
									onChange={(v) =>
										setData({
											...
											visueel: {
												...data.visueel,
												helderheid: v as typeof data.visueel.helderheid,
											},
										})
									}
								/>
								<ButtonGroup
									...
									onChange={(v) =>
										setData({
											...
											visueel: {
												...data.visueel,
												kleur: v as typeof data.visueel.kleur,
											},
										})
									}
									showColor
								...
								<ButtonGroup
									...
									onChange={(v) =>
										setData({
											...
											visueel: {
												...data.visueel,
												belGrootte: v as typeof data.visueel.belGrootte,
											},
										})
									}
								/>
									...
									onChange={(v) =>
										setData({
											...
											visueel: {
												...data.visueel,
												belPersistentie:
													v as typeof data.visueel.belPersistentie,
											},
										})
									}
								/>
								<ButtonGroup
									...
									onChange={(v) =>
										setData({
											...
											visueel: {
												...data.visueel,
												mousse: v as typeof data.visueel.mousse,
											},
										})
									}
								/>
								<Textarea
									...
									placeholder={
										lang === "en"
											? "Other visual notes..."
											: "Andere visuele notities..."
									}
									value={data.visueel.overig || ""}
									onChange={(e) =>
										setData({
											...data,
											visueel: { ...data.visueel, overig: e.target.value },
										})
									}
									rows={2}
								/>
						...
						<Card>
							...
							>
								...
								>
									...
									<Textarea
										placeholder={L.eersteIndruk_placeholder}
												...
												<button
													...
													onClick={() =>
														setData({
															...
															neus: {
																...data.neus,
																autolytischKarakter: opt.waarde,
															},
														})
													}
													style={{
								...
								<div>
									...
									<ChampagneAromaPicker
										geselecteerd={data.neus.aromas}
										onChange={(a) =>
											setData({ ...data, neus: { ...data.neus, aromas: a } })
										}
									/>
								</div>
								<Textarea
									label={L.overigeNeusnoten}
									placeholder={L.aanvullendePalate_placeholder}
									value={data.neus.overig || ""}
									onChange={(e) =>
										setData({
											...data,
											neus: { ...data.neus, overig: e.target.value },
										})
									}
									rows={2}
								/>
						...
						<Card>
							...
							>
								<ButtonGroup
									...
									onChange={(v) =>
										setData({
											...
											mondgevoel: {
												...data.mondgevoel,
												aanval: v as typeof data.mondgevoel.aanval,
											},
										})
									}
								/>
								<ButtonGroup
									label={L.zoetheid}
									opties={
										champagneZoetheidOpties as unknown as {
											waarde: string;
											label: string;
										}[]
									}
									...
									onChange={(v) =>
										setData({
											...
											mondgevoel: {
												...data.mondgevoel,
												zoetheid: v as typeof data.mondgevoel.zoetheid,
											},
										})
									}
									size="sm"
								...
								<ButtonGroup
									label={L.zuurgraad}
									opties={
										champagneZuurgraadOpties as unknown as {
											waarde: string;
											label: string;
										}[]
									}
									...
									onChange={(v) =>
										setData({
											...
											mondgevoel: {
												...data.mondgevoel,
												zuurgraad: v as typeof data.mondgevoel.zuurgraad,
											},
										})
									}
								/>
								<ButtonGroup
									...
									onChange={(v) =>
										setData({
											...
											mondgevoel: {
												...data.mondgevoel,
												body: v as typeof data.mondgevoel.body,
											},
										})
									}
								/>
									...
									onChange={(v) =>
										setData({
											...
											mondgevoel: {
												...data.mondgevoel,
												smaakIntensiteit:
													v as typeof data.mondgevoel.smaakIntensiteit,
											},
										})
									}
								/>
								<div>
									...
									<ChampagneAromaPicker
										...
										onChange={(a) =>
											setData({
												...data,
												mondgevoel: { ...data.mondgevoel, smaakAromas: a },
											})
										}
										neusAromas={data.neus.aromas}
									/>
								</div>
								<ButtonGroup
									...
									onChange={(v) =>
										setData({
											...
											mondgevoel: {
												...data.mondgevoel,
												afdronkLengte:
													v as typeof data.mondgevoel.afdronkLengte,
											},
										})
									}
								/>
								<ButtonGroup
									...
									onChange={(v) =>
										setData({
											...
											mondgevoel: {
												...data.mondgevoel,
												complexiteit: v as typeof data.mondgevoel.complexiteit,
											},
										})
									}
								/>
								<Textarea
									label={L.overigeMondgevoelnoten}
									placeholder={L.aanvullendePalate_placeholder}
					...
					<TabsContent value="conclusie">
						<Card>
							...
							>
								...
								<ButtonGroup
									...
									onChange={(v) =>
										setData({
											...
											conclusie: {
												...data.conclusie,
												drinkbaarheid: v as typeof data.conclusie.drinkbaarheid,
											},
										})
									}
									size="sm"
								/>
								<Textarea
									label={L.rijpingspotentieel}
									placeholder={L.rijpingspotentieel_placeholder}
									...
									onChange={(e) =>
										setData({
											...
											conclusie: {
												...data.conclusie,
												rijpingspotentieel: e.target.value,
											},
										})
									}
									rows={2}
								/>
								<Textarea
									label={L.spijscombinatie}
									placeholder={L.spijscombinatie_placeholder}
