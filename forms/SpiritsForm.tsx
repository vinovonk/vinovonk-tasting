			...
			>
				{filtered.map((a) => (
					<button
						...
						type="button"
						onClick={() => toggle(a)}
						style={pillStyle(selected.includes(a))}
					>
						{a}
					</button>
				))}
...
export const SpiritsForm = forwardRef<SpiritsFormHandle, Props>(
	function SpiritsForm(
		...
		const [data, setData] = useState<SpiritsProef>(
		...
		const handleSave = () => {
			...
			if (!data.uiterlijk.helderheid)
				missing.push(`${L.tabUiterlijk} → ${L.helderheid}`);
			if (!data.uiterlijk.intensiteit)
				missing.push(`${L.tabUiterlijk} → ${L.kleurintensiteit}`);
			...
			if (!data.neus.intensiteit)
				missing.push(`${L.tabNeus} → ${L.intensiteit}`);
			if (!data.gehemelte.zoetheid)
				missing.push(`${L.tabGehemelte} → ${L.zoetheid}`);
			if (!data.gehemelte.smaakIntensiteit)
				missing.push(`${L.tabGehemelte} → ${L.smaakintensiteit}`);
			if (!data.gehemelte.afdronk.lengte)
				missing.push(`${L.tabGehemelte} → ${L.afdronk}`);
			if (!data.conclusie.kwaliteit)
				missing.push(`${L.tabConclusies} → ${L.kwaliteitsniveau}`);
			if (missing.length > 0) {
				toast.error(
					L.missingFieldsSimple(
						missing.length,
					...
					>
						...
						>
							<Input
								label={L.naam}
								placeholder={
									lang === "en" ? "E.g. Glenfiddich 18" : "Bijv. Glenfiddich 18"
								}
								value={data.naam}
							...
							<Input
								label={L.merk}
								placeholder={
									lang === "en" ? "E.g. Glenfiddich" : "Bijv. Glenfiddich"
								}
								value={data.merk || ""}
						...
						>
							<Input
								label={L.spiritLand}
								placeholder={lang === "en" ? "Scotland" : "Schotland"}
								value={data.land || ""}
								onChange={(e) => setData({ ...data, land: e.target.value })}
							/>
							<Input
								label={L.leeftijd}
								placeholder={lang === "en" ? "12 years" : "12 jaar"}
								value={data.leeftijd || ""}
								onChange={(e) => setData({ ...data, leeftijd: e.target.value })}
							/>
							<Input
								label={L.alcoholPercentage}
								type="number"
								step="0.1"
								placeholder="40"
								...
								onChange={(e) =>
									setData({
										...data,
										alcoholPercentage: parseFloat(e.target.value) || undefined,
									})
								}
							/>
							<Input
						...
						<ButtonGroup
							label={L.typeSpirit}
							opties={
								spiritTypeOpties as unknown as {
									waarde: string;
									label: string;
								}[]
							}
							waarde={data.type}
							onChange={(v) => setData({ ...data, type: v as SpiritType })}
							size="sm"
						/>
					</CardContent>
				</Card>
							...
							>
								...
								<ButtonGroup
									...
									onChange={(v) =>
										setData({
											...data,
											uiterlijk: { ...data.uiterlijk, kleur: v },
										})
									}
								/>
							</CardContent>
								...
								>
									...
									{Object.entries(spiritsAromaCategorieen).map(([key, cat]) => (
										<AromaCategoryPicker
											...
											aromas={cat.aromas}
											selected={
												(data.neus.aromaKenmerken as Record<string, string[]>)[
													key
												] || []
											}
											onChange={(v) =>
						...
						<Card>
							...
							>
								<ButtonGroup
									label={L.zoetheid}
									opties={
										spiritsZoetheidOpties as unknown as {
											waarde: string;
											label: string;
										}[]
									}
								...
								>
									...
									{Object.entries(spiritsAromaCategorieen).map(([key, cat]) => (
										<AromaCategoryPicker
											...
											selected={
												(
													data.gehemelte.smaakKenmerken as Record<
														string,
														string[]
													>
												)[key] || []
								...
								<ButtonGroup
									...
									opties={
										spiritsAfdronkLengteOpties as unknown as {
											waarde: string;
											label: string;
										}[]
									}
										...
										setData({
											...
											gehemelte: {
												...
												afdronk: {
													...data.gehemelte.afdronk,
													complexiteit:
														v as typeof data.gehemelte.afdronk.complexiteit,
												},
											},
										})
									}
								...
								<ButtonGroup
									...
									onChange={(v) =>
										setData({
											...data,
											conclusie: {
												kwaliteit: v as typeof data.conclusie.kwaliteit,
											},
										})
									}
								/>
								<Textarea
									label={L.persoonlijkeNotities}
									placeholder={L.jePersoonlijkeObservaties}
									value={notitie}
