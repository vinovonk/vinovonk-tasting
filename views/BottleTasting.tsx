...
import {
  createEmptyWineTasting, createEmptyChampagneTasting,
  createEmptySpiritsTasting, createEmptyAlcoholVrijTasting, createEmptyGenericTasting,
} from '../types';
...
const backBtnStyle: React.CSSProperties = {
  ...
  marginBottom: '1.5rem', padding: 0, display: 'flex', alignItems: 'center', gap: '0.25rem',
};
...
export function BottleTasting({ sessionId, flesId, lang = 'nl' }: BottleTastingProps) {
  const t = T[lang];
  ...
  const [drankType, setDrankType] = useState<DrankType>(() => fles?.drankType ?? 'wijn');
  ...
  const initialAlcoholVrijData = useMemo((): AlcoholVrijTasting => {
    if (fles?.drankType !== 'alcoholvrij') return createEmptyAlcoholVrijTasting();
    return { ...createEmptyAlcoholVrijTasting(), ...(fles.tastingData as Partial<AlcoholVrijTasting>) };
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  const initialGenericData = useMemo((): GenericTasting => {
    if (fles?.drankType !== 'bier' && fles?.drankType !== 'sake' && fles?.drankType !== 'anders') return createEmptyGenericTasting();
    return { ...createEmptyGenericTasting(), ...(fles.tastingData as Partial<GenericTasting>) };
  }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleSave = async (data: TastingData, notitie?: string, score?: number) => {
    setSaving(true);
  ...
  if (!fles) {
    return (
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem var(--gap)' }}>
        <button onClick={() => navigate(`/sessie/${sessionId}`)} style={backBtnStyle}>
          ← {t.back}
        </button>
        <p style={{ fontFamily: 'var(--font-body)', color: 'var(--color-gray)' }}>{t.nieGevonden}</p>
      </div>
    );
  }
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem var(--gap)' }}>
      ...
      {isTweeFase(drankType) && fase === 'proeven' && (
        <div style={{ marginBottom: '1rem' }}>
          <Button variant="outline" onClick={() => setFase('info')}>{t.terugInfo}</Button>
        </div>
      )}
