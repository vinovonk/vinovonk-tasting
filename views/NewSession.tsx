...
export function NewSession({ lang = 'nl' }: NewSessionProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!naam.trim()) return;
    setLoading(true);
    const session = createSession(naam.trim(), datum, beschrijving.trim() || undefined);
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', padding: '2rem var(--gap)' }}>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.5rem)', margin: '0 0 0.5rem' }}>{t.title}</h1>
        <p style={{ fontFamily: 'var(--font-body)', color: 'var(--color-gray)', margin: 0 }}>{t.subtitle}</p>
      </div>
