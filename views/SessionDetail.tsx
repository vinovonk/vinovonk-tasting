...
const T = {
	nl: {
		...
		verwijderOmschrijving:
			"Dit verwijdert de sessie en alle proefnotities permanent. Dit kan niet ongedaan worden gemaakt.",
		annuleer: "Annuleer",
	...
	en: {
		...
		verwijderOmschrijving:
			"This permanently deletes the session and all tasting notes. This cannot be undone.",
		annuleer: "Cancel",
		bevestig: "Yes, delete",
...
export function SessionDetail({ id, lang = "nl" }: SessionDetailProps) {
	...
	const handleDeleteFles = (flesId: string) => {
		...
		setSessie({
			...sessie,
			flessen: sessie.flessen.filter((f) => f.id !== flesId),
		});
		...
		toast.success(`"${flesNaam}" verwijderd`, {
			...
			action: {
				...
				onClick: () => {
					...
					setSessie((prev) =>
						prev
							? { ...prev, flessen: [...prev.flessen, verwijderdeFles] }
							: prev,
					);
		...
		>
			...
			>
				...
				>
					...
					{sessie.flessen.length === 1 ? t.fles : t.flessen}
				</p>
