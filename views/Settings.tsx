...
const T = {
	nl: {
		...
		importeerUitleg:
			"Selecteer een eerder geëxporteerd JSON-bestand. Bestaande sessies worden niet overschreven.",
		importeerGelukkt: "geïmporteerd",
		importeerFout: "Import mislukt",
		importeerSessies: "sessies",
		wis: "Verwijder alle data",
		wisBevestig:
			"Weet je zeker dat je ALLE proefnotities wilt verwijderen? Dit kan niet ongedaan worden gemaakt.",
	...
	en: {
		...
		importeerUitleg:
			"Select a previously exported JSON file. Existing sessions will not be overwritten.",
		importeerGelukkt: "imported",
		importeerFout: "Import failed",
		importeerSessies: "sessions",
		wis: "Delete all data",
		wisBevestig:
			"Are you sure you want to delete ALL tasting notes? This cannot be undone.",
		...
		reader.onload = (ev) => {
			...
			try {
				...
				if (errors.length > 0) {
					...
					toast.success(
						`${imported} ${t.importeerSessies} ${t.importeerGelukkt}`,
					);
				}
			} catch (err) {
				toast.error(
					`${t.importeerFout}: ${err instanceof Error ? err.message : String(err)}`,
				);
			}
		};
