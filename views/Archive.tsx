...
import { getSessions, getSession, exportAllData } from '../lib/storage';
import type { DrankType, TastingNote } from '../types';
import { Card, CardContent } from '../ui/Card';
import { Input } from '../ui/Input';
import { Badge } from '../ui/Badge';
import { Search, Download, Wine, Droplets, Beer, Leaf, HelpCircle } from 'lucide-react';
...
const DRANK_FILTER_OPTIES: { waarde: DrankType | 'alle'; nl: string; en: string }[] = [
...
export function Archive({ lang = 'nl' }: ArchiveProps) {
  ...
  const allNotes = useMemo<FlatNote[]>(() => {
    ...
    // Sort newest first
    return result.sort((a, b) => new Date(b.fles.createdAt).getTime() - new Date(a.fles.createdAt).getTime());
  }, []);

  const filtered = useMemo(() => {
    let items = allNotes;
    if (typeFilter !== 'alle') items = items.filter((n) => n.fles.drankType === typeFilter);
    if (scoreFilter === 'geScoord') items = items.filter((n) => n.fles.score != null);
    if (zoekterm.trim()) {
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem var(--gap)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem', gap: '1rem', flexWrap: 'wrap' }}>
        <div>
          <h2 style={{ fontFamily: 'var(--font-headline)', fontSize: '1.5rem', fontWeight: 900, textTransform: 'uppercase', margin: 0 }}>
            {t.titel}
          </h2>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.82rem', color: 'var(--color-gray)', margin: '0.25rem 0 0' }}>
            {allNotes.length} {allNotes.length === 1 ? t.notitie : t.notities} {t.totaal}
          </p>
        </div>
        <button
          type="button"
          ...
          style={{
            ...
            padding: '0.5rem 0.875rem', fontFamily: 'var(--font-body)',
            ...
            border: '4px solid var(--color-border)', cursor: 'pointer',
      ...
      <div style={{ position: 'relative', marginBottom: '1rem' }}>
        <Search size={14} style={{ position: 'absolute', left: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--color-gray)', pointerEvents: 'none' }} />
        <input
          type="search"
          placeholder={t.zoek}
          value={zoekterm}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: '0.5rem' }}>
        {DRANK_FILTER_OPTIES.map((opt) => (
          <button key={opt.waarde} type="button" style={pillStyle(typeFilter === opt.waarde)} onClick={() => setTypeFilter(opt.waarde)}>
            {lang === 'en' ? opt.en : opt.nl}
          </button>
        ))}
      </div>
      <div style={{ display: 'flex', gap: '4px', marginBottom: '1.5rem' }}>
        <button type="button" style={pillStyle(scoreFilter === 'alle')} onClick={() => setScoreFilter('alle')}>
          {t.alleScores}
        </button>
        <button type="button" style={pillStyle(scoreFilter === 'geScoord')} onClick={() => setScoreFilter('geScoord')}>
          ★ {t.geScoord}
        </button>
      </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
          <p style={{ fontFamily: 'var(--font-body)', fontWeight: 700, color: 'var(--color-on-surface)', marginBottom: '0.5rem' }}>{t.leeg}</p>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.85rem', color: 'var(--color-gray)' }}>{t.leegSub}</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          {filtered.map((n) => (
            <button
              ...
              type="button"
              onClick={() => navigate(`/sessie/${n.sessionId}/fles/${n.fles.id}`)}
              style={{
                display: 'flex', alignItems: 'center', gap: '0.75rem',
                padding: '0.875rem 1rem', textAlign: 'left',
                background: 'var(--color-white)', border: '4px solid var(--color-border)',
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  ...
                  {n.naam || <span style={{ color: 'var(--color-gray)', fontStyle: 'italic' }}>Naamloos</span>}
                </div>
                <div style={{
                ...
                <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.68rem', color: 'var(--color-gray)', marginTop: '2px' }}>
                  {n.sessionNaam} ·{' '}
                  {(() => {
                    try {
                      return format(new Date(n.sessionDatum), 'd MMM yyyy', { locale });
                    } catch {
              ...
              {/* Score */}
              {n.fles.score != null && (
                <Badge style={{ flexShrink: 0, background: 'var(--color-primary)', color: '#fff', border: '2px solid var(--color-primary)', fontWeight: 900, fontSize: '0.85rem', padding: '0.15rem 0.45rem' }}>
                  {n.fles.score}
                </Badge>
              )}
            </button>
